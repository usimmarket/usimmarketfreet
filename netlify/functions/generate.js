...
// Fonts
    // Use built-in standard font by default to keep PDF size small (Netlify/Lambda response limit).
    const helv = await pdfDoc.embedFont(StandardFonts.Helvetica);
    let font = helv;

    // Only embed custom TTF when we actually need non-ASCII output (KR/TH/KH etc.)
    const looksNonAscii = (v) => typeof v === "string" && /[^\x00-\x7F]/.test(v);
    const needsUnicode = Object.values(payload || {}).some(looksNonAscii);

    if (needsUnicode && fs.existsSync(fontPath)) {
      try {
        pdfDoc.registerFontkit(fontkit);
        const fontBytes = fs.readFileSync(fontPath);
        // subset:true should keep the embedded font small in normal cases
        font = await pdfDoc.embedFont(fontBytes, { subset: true });
      } catch (e) {
        // Fallback to standard font if embedding fails
        font = helv;
      }
    }
